...
<head>
    <link rel="icon" type="image/x-icon" href="{{ asset('cinema.png') }}" />
...
<body>
    ...
    <div class="w-full h-screen flex flex-col relative">
        @php
        // Pastikan variabel $movieData tidak kosong dan memiliki properti backdrop_path
        $backdropPath = $movieData && isset($movieData->backdrop_path) ? $imageBaseUrl . 'original' . $movieData->backdrop_path : "";
        @endphp
        {{-- Gunakan isset untuk memastikan $backdropPath tidak null --}}
        @if ($backdropPath)
        ...
        <div class="w-full bg-transparent h-[96px] drop-shadow-lg flex flex-row items-center z-10">
            ...
            <div class="w-1/3 flex items-center justify-center">
                <a href="/" class="font-bold text-5xl font-quicksand text-white hover:text-develobe-500 duration-200">CINEMATION</a>
            </div>
        ...
        <div class="w-full h-full z-10 flex flex-col justify-center px-20">
            ...
            <span class="font-inter italic text-xl mt-4 text-white max-w-3xl line-clamp-5">{{$tagline}}</span>

            <div class="flex flex-row mt-4 items-center">
                <!-- Rating -->
                <div class="w-20 h-20 rounded-full flex items-center justify-center mr-4" style="background: #003040;">
                    <svg class="-rotate-90 w20` h-20">
                        <circle style="color: #004F80;" stroke-width="8" stroke="currentColor" fill="transparent" r="32" cx="40" cy="40" />
                        <circle style="color: #6FCF97;" stroke-width="8" stroke-dasharray="{{$circumference}}" stroke-dashoffset="{{$progressRating}}" stroke-linecap="round" stroke="currentColor" fill="transparent" r="32" cx="40" cy="40" />
                    </svg>
            ...
            <!-- Play Trailer -->
            @if ($trailerID)
            <button class="w-fit bg-develobe-500 text-white pl-4 pr-6 py-3 font-inter text-xl flex flex-row rounded-lg items-center hover:drop-shadow-lg duration-200" onclick="showTrailer(true)">
